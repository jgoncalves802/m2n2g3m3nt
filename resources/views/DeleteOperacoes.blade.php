...
@section('content')
    <body class="hold-transition sidebar-mini layout-fixed">
       ...
       <form action="{{ route('operacoes.destroy', ['id' => $operacao->id]) }}" method="post">
         @csrf
         @method('DELETE')
          ...
          <div class="container card-body col-md-12" style="margin-top:10px; margin-bottom:10px;">
            ...
            <h3>Operação</h3>
              <div class="row">
                ...
                <div class="form col-sm-2" >
                  <label>Total</label>
                  <input type="text" class="form-control " name="Total" value="{{$operacao->Total}}" placeholder="Resultado (%)">
                </div>
                <div class="form col-sm-2" >
                  <label>Nome Estratégia</label>
                  <select  class="form-control badge" name="name_estrategia" id="name_estrategia"  style="font-size: 12px;">
              ...
              <button type="submit" class="btn btn-danger" style="margin-top: 30px; ">Deletar Operação</button>
            </div>
          </div>
        </form>
          <div class="card">
              ...
              <div class="card-body col-md-12">
                <form action="{{ route('operacoes.destroyall', ['email' => $operacao->email]) }}" method="post">
                    @csrf
                    @method('DELETE')
                        <button type="submit" class="btn btn-dark"  style="margin-bottom: -30px; float:right;">Limpar Banco de Dados</button>
                </form>
             </div>
              <!-- /.card-header -->
              <div class="card-body col-md-12">
